...
// Function to know actual size of an array row without counting the -1 for arrays of type int

int tamano_real_double (double* matriz, int ncolumnas, int nfila);
// Function to know actual size of an array row without counting the -1 for double arrays
...
static PyObject* hello_numpy_c(PyObject *dummy, PyObject *args) {
    ...
    // Objects are created to save in them the variables that come from Python once it has been parsed
    ...
    // The number of experiments will match the number of values in the t_f matrix since there is one t_f
    //per experiment. So calculating the number of elements in t_f_obj I get the number of experiments
    ...
    for (int i = 0; i < 15; i++){
        task_py[i] = ((char*)PyUnicode_AsUTF8(task_obj))[i]; // I save the values of the array
    }
    ...
    if(strcmp(task_py,"sim_CVODES_ODE")==0){
        ...
        type_return = 1;
        // To be able to indicate later that the data that is returned to Python is a double
    }else if(strcmp(task_py,"OBS_CVODES_ODE")==0){
        res2=sim_obs_task(amigo_problem);
        type_return = 2;
        // To be able to indicate later that the data that is returned to Python is a list of arrays of doubles
    }
    free (task_py);
...
AMIGO_model* Py_AMIGOmodelAlloc(PyArrayObject *n_states_obj, PyArrayObject *x_obj, PyArrayObject *n_stimuli_obj,
    ...
    int t_s_nelementos = PyArray_SIZE(t_s_obj); // Number of total elements in the t_s matrix
    double *t_s_array = (double*) malloc(sizeof(double)*t_s_nelementos); // I save memory for the array
    for (int i = 0; i < t_s_nelementos; i++){
        t_s_array[i] = ((double*) PyArray_DATA(t_s_obj))[i]; // I save the values of the array
    }
    int t_s_ncolumnas = PyArray_DIMS(t_s_obj)[1]; // I calculate the number of elements per row (including -1)
    n_times = tamano_real_double (t_s_array, t_s_ncolumnas, exp_num); // I calculate the number of elements in that row without -1

    n_controls = *((int*) PyArray_DATA(n_stimuli_obj));

    int t_con_nelementos = PyArray_SIZE(t_con_obj); // Number of total elements in the t_con matrix
    double *t_con_array = (double*) malloc(sizeof(double)*t_con_nelementos); // I save memory for the array
    for (int i = 0; i < t_con_nelementos; i++){
        t_con_array[i] = ((double*) PyArray_DATA(t_con_obj))[i]; // I save the values of the array
    }
    int t_con_ncolumnas = PyArray_DIMS(t_con_obj)[1]; // I calculate the number of elements per row (including -1)
    n_controls_t = tamano_real_double (t_con_array, t_con_ncolumnas, exp_num);
    // I calculate the number of elements in that row without -1
    ...
    amigo_model=allocate_AMIGO_model(n_states,n_observables,n_pars,
  	...
  	int index_observables_nelementos = PyArray_SIZE(index_observables_obj); // Number of total elements
    ...
    for (int i = 0; i < index_observables_nelementos; i++){
        index_observables_array[i] = ((int*) PyArray_DATA(index_observables_obj))[i]; // I save the values of the array
    }
    int index_observables_ncolumnas = PyArray_DIMS(index_observables_obj)[1];
    // I calculate the number of elements per row (including -1)
	...
	//Initial conditions
	int y0_nelementos = PyArray_SIZE(y0_obj); // Number of total elements in the y0 matrix
    double *y0_array = (double*) malloc(sizeof(double)*y0_nelementos); // I save memory for the array
    for (int i = 0; i < y0_nelementos; i++){
        y0_array[i] = ((double*) PyArray_DATA(y0_obj))[i]; // I save the values of the array
    }
    int y0_ncolumnas = PyArray_DIMS(y0_obj)[1]; // I calculate the number of elements per row (including -1)
    ...
    //Control values
    counter = 0;
    int u_nelementos = PyArray_SIZE(u_obj); // Number of total elements in the matrix u
    double *u_array = (double*) malloc(sizeof(double)*u_nelementos); // I save memory for the array
    for (int i = 0; i < u_nelementos; i++){
        u_array[i] = ((double*) PyArray_DATA(u_obj))[i]; // I save the values of the array
    }
    int u_ncolumnas = PyArray_DIMS(u_obj)[1]; // I calculate the number of elements per row (including -1)
    ...
    //Experimental data
	counter=0;
	int exp_data_nelementos = PyArray_SIZE(exp_data_obj); // Number of total elements in the exp_data matrix
    double *exp_data_array = (double*) malloc(sizeof(double)*exp_data_nelementos); // I save memory for the array
    for (int i = 0; i < exp_data_nelementos; i++){
        exp_data_array[i] = ((double*) PyArray_DATA(exp_data_obj))[i]; // I save the values of the array
    }
    int exp_data_ncolumnas = PyArray_DIMS(exp_data_obj)[1]; // I calculate the number of elements per row (including -1)
